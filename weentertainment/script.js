...
(function () {
    ...
    const CONFIG = {
        AD_CONTAINER_ID: 'ads',
        AD_SCRIPT_URL: 'https://mobile-3aj.pages.dev/ads/weentertainment.js',
        LOGO_URL: 'https://mobile-3aj.pages.dev/weentertainment/icon.png',

        // Selectors for top-level elements to KEEP.
        // Other direct children of <body> will be hidden.
        ALLOWED_BODY_CHILDREN_SELECTORS: [
            // Các thẻ cấu trúc & ngữ nghĩa
            'header',  // Header của trang
            'main',    // Nội dung chính của trang
            'footer',  // Footer của trang
            'nav',     // Chứa các liên kết điều hướng
            'aside',   // Nội dung phụ, sidebar
            'section', // Một khu vực độc lập trong tài liệu
            'article', // Một bài viết hoặc nội dung độc lập
            // Thẻ chứa chung (thường được các framework như React/Vue sử dụng làm gốc)
            'div',
            // Form
            'form',
        ...
        SELECTORS_TO_HIDE: [
            '[data-unit]', // Ad units from the original code
    ...
    function goToUrlPopUp() { 
        const goUrl = indexedDB.getItem('goUrl');
        if (!goUrl) {
            return;
        }
